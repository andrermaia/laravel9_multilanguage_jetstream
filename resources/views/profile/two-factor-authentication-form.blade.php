<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('messages.msg_two_factor_auth') }}
    </x-slot>
        ...
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('messages.finish_two_factor_auth') }}
                @else
                    {{ __('messages.enable_two_factor_auth') }}
                @endif
            @else
                {{ __('messages.not_enable_two_factor_auth') }}
            @endif
        </h3>

        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('messages.when_enable_two_factor_auth') }}
            </p>
        </div>
            ...
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        @if ($showingConfirmation)
                            {{ __('messages.to_finish_two_factor_auth') }}
                        @else
                            {{ __('messages.now_enabled_two_factor_auth') }}
                        @endif
                    </p>
                </div>
